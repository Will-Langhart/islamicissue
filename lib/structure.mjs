...
import { editorialFor, extractEvidence } from "./editorial.mjs";
import { part1AuditFor } from "./part1-audit.mjs";
import { reviewForIssue } from "../content/review-workflow.mjs";
...
export const site = parts.map((part, pi) => {
  ...
  return {
    ...
    items: part.items.map((item, ii) => ({
      ...
      editorial: item.editorial || editorialFor(meta.slug),
      evidence: item.evidence || extractEvidence([
        ...(item.critique || []),
        ...(item.response || []),
        ...(item.rebuttal || []),
      ]),
      audit: meta.slug === "islamic-dilemma" ? part1AuditFor(slugify(item.title)) : null,
      review: reviewForIssue(meta.slug, slugify(item.title)),
    })),
  };
});
...
export const searchIndex = [
  ...
  { href: "/methodology", title: "Methodology and Evidence Standards", label: "Editorial standards", text: "internal critique steelman responses evidence confidence corrections source audit" },
  { href: "/timeline", title: "Text and Tradition Timeline", label: "Visual chronology", text: "manuscripts Quran canon Uthman hadith Bukhari Muslim readings Cairo" },
  { href: "/graph", title: "Knowledge Graph Lab", label: "Graph explorer", text: "network graph concepts related issues clustering citations nodes edges" },
  { href: "/citations", title: "Citation Validation Report", label: "Citation analytics", text: "citation coverage validation quran hadith scholar warnings malformed references" },
  { href: "/corrections", title: "Corrections and Revision Log", label: "Editorial transparency", text: "corrections revisions report error evidence" },
];
